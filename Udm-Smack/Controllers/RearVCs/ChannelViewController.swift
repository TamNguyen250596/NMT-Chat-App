...
class ChannelViewController: UIViewController {
    ...
    @IBOutlet weak var userImge: CircleImage!
    ...
    func updateUserInfo() {
        if AuthorService.instance.isLoggedin {
            ...
            userImge.image = UIImage(named: UserDataService.instance.avatarName)
            userImge.backgroundColor = String().returnColor(components: UserDataService.instance.avatarColor)
            checkedLogin = true
        } else {
            loginBtn.switchLanguagesForButton(titleBtn: "Login", color: .white, fontType: .helvetica, fontsize: 20)
            userImge.image = UIImage(named: "smackProfileIcon")
            userImge.backgroundColor = UIColor.lightGray
